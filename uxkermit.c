...
#include "uxkermit.h" // Conditional compilation for different machines/operating systems
...
struct	sgttyb oldtty;		/* Controlling tty raw mode */
...
/*  Done parsing */
    ...
    if (ttyname) {			/* If LINE was specified, we  operate in local mode */
	...
	ttyfd = ttopen(0);    		/* in remote mode (ie. controlling */
	ttbin(ttyfd, &oldtty);		/* tty is the communications line) */
	remote = TRUE;			/* Put the tty in binary mode */
    }
...
#if UCB4X | UZI180			/* TOPS-20 can't handle timeouts... */
